...
repository to GradeScope at the "Java Assessment Exercise" link.  Once you
...
Don't forget that you have to Push your changes to upload them to the
repository.

## GradeScope Feedback

There are 10 tests for this exercise and if there is an error, the error
message will tell you what was expected what was observed.  When the compared
value is a string, brackets ([, ]) are used to annotate exactly which part of
the two strings differed.  If all the SortedCollectionTest tests passed, then
you should not get any deductions because GradeScope runs the exact same tests.
Feel free to resubmit until you get a perfect score.  ## Resources
